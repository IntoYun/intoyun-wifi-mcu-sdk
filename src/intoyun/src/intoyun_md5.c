...
static uint8_t _md5Buffer[16]; //存储转换结果
...
void md5_output(uint8_t *data, uint16_t len, char *signature)
{
    ...
    for(uint8_t i = 0; i < 16; i++) {
        sprintf(output + (i * 2), "%02x", _md5Buffer[i]);
    }
    log_v_dump(_md5Buffer,16);
    log_v("output=%s\r\n",output);
    memcpy(signature,output,strlen(output));
}
